...
def make_fallback_article(title):
    ...
    sections = [
        ("Einführung", "In diesem Beitrag bekommst du einen schnellen, praxisnahen Überblick."),
        ("Worauf es ankommt", "<ul><li>Performance & Zuverlässigkeit</li><li>Kostenkontrolle</li></ul>"),
        ("Fazit", "<ul><li>Klein starten, regelmäßig verbessern</li><li>Sicherheit beachten</li></ul>"),
    ]
    body = [f"<h1>{title}</h1><p class='lead'>{meta}</p>"]
    for h, html in sections:
        body.append(f"<h2>{h}</h2><p>{html}</p>")
    html = "\n".join(body)
    tags = ["Hosting","Cloud","Basics"]
...
def generate_article(title):
    ...
    user = f"""Gib mir ausschließlich ein JSON-Objekt mit: title, meta, tags[], html (<article>…</article>).
Thema: {title}"""
